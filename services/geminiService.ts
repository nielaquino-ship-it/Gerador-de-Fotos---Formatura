...
export async function generateGraduationPhoto(base64Image: string, mimeType: string, customText: string): Promise<string> {
  try {
    const textPrompt = `Edite esta foto de um estudante. Adicione uma beca de formatura preta com uma faixa azul. Adicione também um capelo (chapéu de formatura) preto na cabeça. O estilo deve ser realista, mantendo o fundo e o rosto originais.${customText ? ` MUITO IMPORTANTE: Na parte inferior da imagem, escreva o texto exatamente assim: "${customText}". Preste muita atenção para não cometer erros de digitação. O texto deve ter uma fonte elegante e legível, com cor que contraste bem com o fundo (branco ou amarelo).` : ''} Retorne apenas a imagem finalizada.`;
